...
<nul set /p strTemp=dump public.logs2_%%Y_%%M --
@REM echo dump public.logs_%%Y_%%M
pg_dump.exe --file %~dp0\backups\logs2_%%Y_%%M.backup --host "localhost" --port "5432" --username "postgres" --no-password --format=c --blobs --compress "1" --table "lanmon2.logs2_%%Y_%%M" %dbname%
echo !time!
)
)
echo on
